...
bool intersectsThisTri(CompFab::Triangle &triangle, CompFab::Vec3 dir, CompFab::Vec3 pos)
{
    // 与每一个三角形面片判断是否相交
    CompFab::Vec3 V1(triangle.m_v1.m_x, triangle.m_v1.m_y, triangle.m_v1.m_z);
    CompFab::Vec3 V2(triangle.m_v2.m_x, triangle.m_v2.m_y, triangle.m_v2.m_z);
    CompFab::Vec3 V3(triangle.m_v3.m_x, triangle.m_v3.m_y, triangle.m_v3.m_z);

    //Find vectors for two edges sharing V1
    CompFab::Vec3 e1 = V2 - V1;
    CompFab::Vec3 e2 = V3 - V1;

    //Begin calculating determinant - also used to calculate u parameter dir确定一个采样/观测的方向
    CompFab::Vec3 P = cross(dir, e2);
...
void useCpuToVoxelize(int samples, int w, int h, int d, CompFab::VoxelGrid *g_voxelGrid, std::vector<CompFab::Triangle> triangles, bool double_thick)
{
	...
	CompFab::Vec3 lower_left(g_voxelGrid->m_lowerLeft.m_x, g_voxelGrid->m_lowerLeft.m_y, g_voxelGrid->m_lowerLeft.m_z); // the start of the model
            ...
            for (int i = 0; i < w; ++i)
            {
                for (int j = 0; j < h; ++j)
                {
                    for (int k = 0; k < d; ++k)
                    {
                        ...
                        CompFab::Vec3 pos(lower_left.m_x + spacing * i, lower_left.m_y + spacing * j, lower_left.m_z + spacing * k);
                        CompFab::Vec3 dir;
                        ...
                        for (int sample = 0; sample < samples; ++sample)
                        {
                            ...
                            dir.m_x = (sqrt(1 - z * z) * cosf(theta));
                            dir.m_y = (sqrt(1 - z * z) * sinf(theta));
                            dir.m_z = (sqrt(1 - z * z) * cosf(theta));
